	...
	getPool().getConnection(function(err, db){
		...
		var sql = "SELECT f.*, breaker, CONCAT(score,'.',firsts,'.',seconds,'.',thirds,'.',fourths,'.',subbed,'.',IFNULL(breaker,0)) AS scoring_string " +
			...
			"LEFT JOIN (  " +
				...
				"WHERE  " +
					// "starts = "+(starts != null ? "'"+starts+"' " : "(SELECT MAX(starts) FROM league_night WHERE season_id="+season_id+") ") +
...
exports.getOrderForNight = function(season_id, starts){
	...
	getPool().getConnection(function(err, db){
		...
		var sql = "SELECT " +
			"lno.rank, lno.start_order, lno.grouping, lno.dnp, p.*, IFNULL(f.score, 0) AS score, IFNULL(CONCAT(score,'.',firsts,'.',seconds,'.',thirds,'.',fourths,'.',subbed,'.',IFNULL(breaker,0)),0) AS scoring_string " +
		"FROM " +
		"league_night n " +
		...
		"ORDER BY " +
			"lno.start_order";
		db.query(sql, [season_id, starts, starts, starts], function(err, results){
...
exports.getPointsForNight = function(season_id, starts){
	...
	.then(function(player_list){
			...
			getPool().getConnection(function(err, db){
				...
				var sql = "SELECT " +
						"lns.name_key, SUM(lns.points) AS points, GROUP_CONCAT(mtln.abbv,':',IFNULL(lns.points,'') ORDER BY mtln.display_order) AS machines " +
					"FROM " +
						"player_to_season pts " +
					 	"JOIN league_night n USING(season_id) " +
						"JOIN machine_to_league_night mtln USING(night_id) " +
						"JOIN league_night_score lns USING(name_key, night_id, abbv) " +
					"WHERE n.starts = ? " +
