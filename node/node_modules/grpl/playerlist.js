...
exports.getRankings = function(starts){
	...
	getPool().getConnection(function(err, db){
		...
		var sql = 'SELECT p.*, SUM(n.points) AS score, SUM(n.firsts) AS firsts, SUM(n.seconds) AS seconds, SUM(n.thirds) AS thirds, SUM(n.fourths) AS fourths, SUM(n.subbed) AS subbed ' +
			'FROM player_points_per_night n ' + 
			'LEFT JOIN player p USING(name_key) ' +
			(starts != null ? 'WHERE starts < \''+starts+'\' ' : '') + 
			'GROUP BY name_key ' +
			'ORDER BY score DESC, firsts DESC, seconds DESC, thirds DESC, fourths DESC, subbed';
		...
		db.query(sql, function(err, results){
			...
			if(results.length > 0){
				...
				var sql = "SELECT p.*, 0 AS score FROM league_night ln LEFT JOIN player_to_season pts USING(season_id) LEFT JOIN player p USING(name_key) WHERE ln.starts=?";
