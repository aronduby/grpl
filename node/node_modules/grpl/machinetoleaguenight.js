...
MachineToLeagueNight.prototype.save = function(){
	...
	getPool().getConnection(function(err, connection){
		...
		for(var abbv in this.machines){
			var data = {
				'abbv': abbv,
				'starts': this.starts,
				'display_order': this.machines[abbv]
			};
		...
		var del_query = connection.query("DELETE FROM machine_to_league_night WHERE starts=? AND abbv NOT IN (?)", [this.starts, keys.join(',')], function(err,result){
			if(err){ d.reject(err); return false; }
			connection.end();
			d.resolve(true);
		});
