...
exports.getByStarts = function(starts){
	...
	getPool().getConnection(function(err, db){
		...
		db.query("SELECT * FROM league_night WHERE starts=?", [starts], function(err, results){
			if(err){ d.reject(err); db.end(); return false; }
			d.resolve(new LeagueNight(results[0]));
			db.end();
		});
	});
...
exports.getAllForSeason = function(season_id){
	...
	getPool().getConnection(function(err, db){
		...
		var sql = "SELECT * FROM league_night WHERE season_id=? ORDER BY starts DESC";
