define(['js/app'], function (app) {
    ...
    var ScoringController = function ($scope, $filter, $state, $stateParams, $localStorage, loadingOverlayApi, dialog, navApi, Auth, Seasons, Scoring, LeagueNights, Players, Machines) {
        ...
        // check for admin setup - user specified, and not the user logged in, and not admin
            ...
            $scope.machineChoosen = false;
            $scope.saveEnabled = false;

            // watch confirmed and machine to enable the save button
            $scope.$watchGroup(['confirmed', 'machine'], function (newVals) {
                $scope.machineChoosen = !!_.get(newVals[1], 'abbv');
                $scope.saveEnabled = $scope.confirmed && $scope.machineChoosen;
            });
            ...
            $scope.save = function () {
                ...
                if (_.filter($scope.group.players, function (p) {
                        return p.place != undefined
                    }).length != $scope.group.players.length) {
                    scoringError('You must give everyone a place.');
                    return false;
                }

                var dnps = _.filter($scope.group.players, function (p) {
                    return p.place.points === 0
                });
